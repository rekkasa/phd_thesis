...
effect heterogeneity (**REF**: PATH). Using variation in baseline risk across
patient subgroups, the PATH statement presented comprehensive guidance on
performing risk-based evaluation of treatment effect heterogeneity. It also
identified several knowledge gaps that need to be addressed in future research.
Among many others, the need for further research in the evaluation of treatment
effect heterogeneity in observational rather than experimental/randomized data
and methods for individualization of predicted treatment effect were
highlighted.
...
\textbf{Demonstration}
\begin{enumerate}
\item \textit{Compare the effect of thiazide or thiazide-like diuretics to the effect of ACE inhibitors in patients with established hypertension with respect to 12 outcomes (acute myocardial infarction; hospitalization with heart failure; ischemic or hemorrhagic stroke; acute renal failure; kidney disease; cough; hyperkalemia; hypokalemia; gastrointestinal bleeding; hyponatremia; hypotension; angioedema).}
\item\textit{IBM MarketScan Commercial Claims and Encounters, IBM MarketScan Multi-State Medicaid, and IBM MarketScan Medicare Supplemental Beneficiaries.}
\item\textit{In each database, develop prediction models for acute myocardial infarction on the 1:1 propensity score-matched subset of the pooled thiazide or thiazide-like diuretics and ACE inhibitor treatment arms.}
\item\textit{In each database, stratify the population in patients below 1\%, between 1\% and 1.5\% and above 1.5\% acute myocardial infarction risk. Within risk groups estimate relative treatment effects from proportional hazards models and absolute treatment effects from the difference of Kaplan-Meier estimates on day 730 after treatment initiation. Adjust for propensity scores for all estimates.}
\item\textit{Overall treatment effects favoring ACE inhibitors were mainly driven by patients with predicted acute myocardial risk above 1.5\%.}
\end{enumerate}
\end{minipage}

\end{blackbox}

With the development of the framework presented in chapter @sec-framework our
aim was to enable the evaluation of treatment effect heterogeneity using readily
available observational patient data. Despite its great promise, observational
...
apply to individuals. In chapter \@ref(simulation), we compared regression-based
...
(**REF: Hoogland et al**). Although models allowing for individual
...
Patient populations can vary substantially over time as they become older, In
addition, the characteristics of populations located in different places may be
...
validated the EORTC-DeCOG nomgram of chapter @sec-melanoma in data from Germany
and adopted a leave-one-center-out validation approach for the COPE prediction
model of chapter 4 (**REF**: internal-exetrnal paper Ewout).
...
observational setting (**REFS**). A head-to-head comparison of these methods and
the proposed risk-based approaches can provide further insights into their
relative performance and help guide model selection and implementation in
different settings.
