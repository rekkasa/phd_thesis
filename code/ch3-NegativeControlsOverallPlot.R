...
negativeControls <- readRDS("data/framework/mappedOverallResultsNegativeControls.rds") %>%
  filter(
    !is.na(seLogRr),
    analysisType == "ps_strat_acute_myocardial_infarction"
  ) %>%
  mutate(database = toupper(database))

plotList <- list()
for (i in seq_along(dbs)) {
  ncs <- negativeControls %>%
    filter(database == dbs[i])
...
fileName <- "ch3-NegativeControlsOverall.tiff"
ggsave(
  file.path(
    "figures",
    fileName
