...
risk-based treatment effect (Supplement, Sections 2 and 3) [@kent2016risk].
...
subgroup analyses [@kent2019predictive1;@kent2019predictive2]. Patients are
stratified into equally-sized risk strata—in this case based on risk quartiles.
Absolute treatment effects, within risk strata, expressed as absolute risk
differences, are estimated by the difference in event rate between control and
treatment arm patients. We considered this approach as a reference, expecting it
to perform worse than the other candidates, as its objective is to provide an
illustration of HTE rather than to optimize individualized benefit predictions.
...
flexibility (Supplement, section 4).
...
indication, to predict 30-day mortality risk (Supplement, Section 10). Predicted
baseline risk is derived by setting the treatment indicator to 0 for all
patients.
...
work [@glasziou1995evidence]. In a limited set of scenarios with true interactions between
